...
export function buildFormRequest(
  ...
  const submitControl = submitter instanceof HTMLButtonElement || submitter instanceof HTMLInputElement
    ? submitter
    : undefined;
  const action = submitControl?.hasAttribute("formaction") === true ? submitControl.formAction : form.action;
  const method = (
  ...
  const url = new URL(action || form.ownerDocument.URL, form.ownerDocument.baseURI);

...
| Public props | Attributes, `MutationObserver`, native scalar conversion | Declared type boundary, explicit-only `data-*` reflection, and reflection batching | Props are attributes; no element properties are added. Direct scalar and enum props produce a 1,715-byte gzip fixture. The contract and type boundary share one recursive-freeze implementation. |

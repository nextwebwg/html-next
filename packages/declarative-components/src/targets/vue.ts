...
export function generateVue(definition: ComponentDefinition, version: string): string {
  ...
  const needsLifecycle = needsBridge || target.events.length > 0;
  const define = [
    "defineProps<{",
  ...
  return [
    ...
    `import { ${reactive === undefined ? "" : "computed, "}${needsLifecycle ? "onMounted, onUnmounted, " : ""}ref, watchEffect } from "vue";`,
    ...(dispatchesEvents
    ...
    ...eventListeners,
    ...(needsLifecycle ? [
      ...(needsBridge ? ["let detach: undefined | (() => void);"] : []),
      "onMounted(() => {",

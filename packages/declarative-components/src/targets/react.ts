...
function renderNode(
  ...
  if (node.kind === "slot") {
    const fallbackNodes = node.fallback?.map(
      (child) => renderNode(child, aliases, props, owner, depth, reactive),
    ).join("\n") ?? "";
  ...
  const children = [
    valueChild,
    ...node.children.map((child) => renderNode(child, aliases, props, owner, depth + 1, reactive)),
  ].filter(Boolean).join("\n");
  return `${open}\n${indent}  ${children}\n${indent}</${node.name}>`;
}
...
export function generateReact(definition: ComponentDefinition, version: string): string {
  ...
  const children = [
    ...
    ...template.children.map((child) =>
      renderNode(child, aliases, contract.props, contract.tag, 2, reactive)
    ),
  ].filter(Boolean).join("\n");
    ...
    ...(reactive === undefined ? [] : [
      ...
      ...reactive.handlers.flatMap((handler) => {
        ...
        for (const step of handler.declaration.steps) {
          ...
          values.set(state.name, "current");
          lines.push(`    ${state.setter}((current) => ${nativeExpression(step.value.ast, values)});`);
        }
        lines.push("  };");
        return lines;
      }),

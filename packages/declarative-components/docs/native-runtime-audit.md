...
be attributed cleanly to individual modules.
...
| Hydration and adoption | Existing DOM identity, selectors, control state, focus, and selection APIs | Matching server-lowered roots to definitions and attaching only authored behavior | Existing tests preserve node identity and live form-control state. A reproducible adoption/hydration benchmark lane remains required before performance claims are complete. |

...
A property declaration gives a public name, type, optional default, requiredness, and binding target. Properties are HTML attributes: the author writes them on the component invocation, and their strings are parsed through the declared type as specified by [Invocation attribute parsing](types-and-validation.md#invocation-attribute-parsing). Any declared type with a text form can be a property. Terminals and keyword unions are written as their text; collection and structured types (`list`, `record`, and `object` shapes) are written as JSON text and parsed against the declared shape as specified by [Collection and structured types](types-and-validation.md#collection-and-structured-types), for example `tags='["design","docs"]'`. `function`, `unknown`, and trusted-content types have no text form and cannot be properties.

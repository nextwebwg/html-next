...
`@nextwebwg/declarative-components` is mechanically prepared for the manual
...
```sh
mkdir -p .release
corepack pnpm --filter @nextwebwg/declarative-components pack --pack-destination .release
npm publish .release/nextwebwg-declarative-components-1.0.0-alpha.0.tgz
```
